...
- 2026-04-17 — D1-02: added 5 materialized-view-layer DDLs per §Materialized views. Additive `events` columns `repo_id_hash Nullable(String)` and `prompt_cluster_id Nullable(String)` added in migration `0002` (closes the gap between §events and §MVs where the columns were assumed but not declared). `cluster_assignment_mv` is a plain `ReplacingMergeTree(ts)` table populated by Workstream H's nightly cluster job (Sprint 2), not a CH `MATERIALIZED VIEW` triggering on `events` — name retained for consistency with this contract. `dev_team_dict` PG-backed dictionary DDL landed; source query returns `NULL::text` for `team_id` until D1-05 adds the `teams` table + `developers.team_id` column. MV tables use `SETTINGS allow_nullable_key = 1` where the ORDER BY includes `Nullable` columns (`team_id`, `repo_id_hash`).
