...
const superUrl = process.env.DATABASE_URL ?? "postgres://postgres:postgres@localhost:5435/bematist";
...
beforeAll(async () => {
  ...
  async function seedOrg(org: { id: string }, tag: string) {
    ...
    await superDb
      .insert(git_events)
      .values({ org_id: org.id, repo_id: repo.id, kind: "pr", pr_number: 1 });
    await superDb.insert(ingest_keys).values({
      org_id: org.id,
      key_prefix: `dm_${tag}`,
      hashed_secret: "x",
      created_by: u.id,
