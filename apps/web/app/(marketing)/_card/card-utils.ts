...
export interface Highlights {
  ...
  mcpServers: Array<{ name: string; totalCalls: number; tools: Array<{ name: string; count: number }> }>
...
const personalityMap: Record<string, { name: string; desc: string }> = {
  '9-to-5er / Power User': { name: 'The Daylight Builder', desc: 'You ship during working hours with relentless consistency.' },
  '9-to-5er': { name: 'The Daylight Builder', desc: 'You ship during working hours with relentless consistency.' },
  'Night Owl / Power User': { name: 'The Midnight Architect', desc: 'You do your best work when the world goes quiet.' },
  'Night Owl': { name: 'The Midnight Architect', desc: 'You do your best work when the world goes quiet.' },
  'Weekend Warrior': { name: 'The Weekend Warrior', desc: 'Saturdays and Sundays are your secret weapon.' },
  'Early Bird': { name: 'The Dawn Shipper', desc: 'First commit before the sun rises. Unstoppable.' },
...
export function mapPersonality(raw: string): { name: string; desc: string } {
  if (personalityMap[raw]) return personalityMap[raw]
...
export function getPersonality(hourDist: number[]): { name: string; desc: string } {
  const night = hourDist.slice(21, 24).reduce((a, b) => a + b, 0) + hourDist.slice(0, 5).reduce((a, b) => a + b, 0)
  ...
  if (nightPct > 0.4) return { name: 'The Midnight Architect', desc: 'You build when the world goes quiet.' }
  if (morningPct > 0.4) return { name: 'The Dawn Shipper', desc: 'First commit before the sun rises.' }
  if (afternoon / total > 0.4) return { name: 'The Daylight Builder', desc: 'Peak focus during working hours.' }
  if (evening / total > 0.4) return { name: 'The Twilight Coder', desc: 'You hit your stride as the day winds down.' }
...
export function getCodexPersonality(stats: CardData['stats']): { name: string; desc: string } {
  ...
  const execCalls = tools.filter(t => ['exec_command', 'shell_command', 'shell', 'write_stdin'].includes(t.name)).reduce((s, t) => s + t.count, 0)
  ...
  if (searches > 200) return { name: 'Research Engineer', desc: `${searches} web searches you let Codex explore before building.` }
  if (reasoningRatio > 150) return { name: 'Deep Reasoner', desc: `${reasoning.toLocaleString()} reasoning blocks Codex thinks hard for you.` }
  if (execRatio > 0.7) return { name: 'Shell Commander', desc: `${execCalls.toLocaleString()} exec calls you live in the terminal.` }
  if (editRatio > 0.3) return { name: 'Patch Artist', desc: `${editCalls.toLocaleString()} patches applied surgical code changes.` }
  if (sessions >= 100) return { name: 'Codex Veteran', desc: `${sessions} sessions across ${modelCount} models a power user.` }
...
export function getAchievements(stats: CardData['stats'], view: 'combined' | 'claude' | 'codex' = 'combined'): Array<{ name: string; desc: string; color: string; icon: AchievementIcon }> {
  const achievements: Array<{ name: string; desc: string; color: string; icon: AchievementIcon }> = []

  const sessions = view === 'claude' ? stats.claude.sessions : view === 'codex' ? stats.codex.sessions : stats.combined.totalSessions
  const activeDays = view === 'codex' ? (stats.codex.activeDays ?? 0) : view === 'claude' ? stats.claude.activeDays : (stats.combined.totalActiveDays ?? stats.claude.activeDays)
...
export function getFunFacts(stats: CardData['stats'], view: 'combined' | 'claude' | 'codex' = 'combined'): Array<{ text: string; color: string }> {
  const facts: Array<{ text: string; color: string }> = []

  const tokens = view === 'claude' ? stats.claude.inputTokens + stats.claude.outputTokens
    : view === 'codex' ? stats.codex.inputTokens + stats.codex.outputTokens
    : getTotalTokens(stats)
  const cost = view === 'claude' ? stats.claude.cost : view === 'codex' ? stats.codex.cost : stats.combined.totalCost
  const models = view === 'codex' ? stats.codex.models : view === 'claude' ? stats.claude.models : { ...stats.claude.models }
...
export function getModelDistribution(stats: CardData['stats']): Array<{ name: string; pct: number; color: string }> {
  ...
  for (const [name, data] of Object.entries(stats.codex.models)) {
    if (models[name]) {
      models[name] = { sessions: models[name].sessions + data.sessions, cost: models[name].cost + data.cost }
    } else {
      models[name] = data
    }
  }
