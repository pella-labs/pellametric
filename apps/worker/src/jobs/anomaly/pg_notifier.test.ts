...
// These tests TRUNCATE `alerts` + `orgs` with CASCADE, which would wipe
// `users` / `developers` / `policies` in a shared dev database. Gate on an
// explicit opt-in env var — CI sets `PG_INTEGRATION_TESTS=1` with a
// dedicated disposable Postgres service; local `bun run test` leaves it
// unset so a dev's running stack isn't nuked out from under them.
...
testIf(
  "PostgresAnomalyNotifier persists alerts and pg_notifies subscribers within 5s",
  async () => {
    const org = must(
      (
        await localDb.insert(orgs).values({ slug: "anom_org", name: "Anomaly Test" }).returning()
      )[0],
    );
...
testIf("notifier maps each detector signal to the right alerts.kind discriminator", async () => {
  const org = must(
    (await localDb.insert(orgs).values({ slug: "anomkinds", name: "Kinds" }).returning())[0],
  );
  const notifier = new PostgresAnomalyNotifier({ db: localDb });
